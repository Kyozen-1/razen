...
@section('title', 'Admin - Razen Studio | Admin | Landing Page | Video Production | Video Company Profile | Data | Testimoni')
        ...
        <div class="col-12 col-md-7">
            ...
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item"><a href="#">Video Company Profile</a></li>
                    <li class="breadcrumb-item"><a href="#">Data</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('razen-studio.admin.video-production.video-company-profile.data.testimoni.index') }}">Testimoni</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#testimoni_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-studio.admin.video-production.video-company-profile.data.testimoni.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#testimoni_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('razen-studio.admin.video-production.video-company-profile.data.testimoni.store') }}",
                    method: "POST",
            ...
            {
                $.ajax({
                    url: "{{ route('razen-studio.admin.video-production.video-company-profile.data.testimoni.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-studio.admin.video-production.video-company-profile.data.testimoni.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_gambar').attr('src', "{{ asset('images/landingpage_razenstudio/video-production/video-company-profile/testimoni') }}" + '/' + data.result.gambar);
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-studio.admin.video-production.video-company-profile.data.testimoni.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    var lokasi_gambar = "{{ asset('images/landingpage_razenstudio/video-production/video-company-profile/testimoni') }}"+'/'+data.result.gambar;
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-studio.admin.video-production.video-company-profile.data.testimoni.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            $.ajax({
                url: url,
                beforeSend: function(){
