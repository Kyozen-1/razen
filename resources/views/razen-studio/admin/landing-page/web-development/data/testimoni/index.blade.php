...
@section('title', 'Admin - Razen Studio | Admin | Landing Page | Web Development | Data | Testimoni')
        ...
        <div class="col-12 col-md-7">
            ...
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('razen-studio.admin.web-development.data.testimoni.index') }}">Testimoni</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#testimoni_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-studio.admin.web-development.data.testimoni.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#testimoni_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('razen-studio.admin.web-development.data.testimoni.store') }}",
                    method: "POST",
            ...
            {
                $.ajax({
                    url: "{{ route('razen-studio.admin.web-development.data.testimoni.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-studio.admin.web-development.data.testimoni.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_gambar').attr('src', "{{ asset('images/landingpage_razenstudio/home/testimoni') }}" + '/' + data.result.gambar);
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-studio.admin.web-development.data.testimoni.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    var lokasi_gambar = "{{ asset('images/landingpage_razenstudio/home/testimoni') }}"+'/'+data.result.gambar;
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-studio.admin.web-development.data.testimoni.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            $.ajax({
                url: url,
                beforeSend: function(){
