...
@section('title', 'Admin - Razen Studio | Admin | Landing Page | Home | Data | Portofolio')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">Portofolio</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('razen-studio.admin.home.data.portofolio.index') }}">Portofolio</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="portofolio_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="portofolio_form" method="POST" enctype="multipart/form-data">
                        @csrf
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#portofolio_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-studio.admin.home.data.portofolio.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#portofolio_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-studio.admin.home.data.portofolio.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#portofolio_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#portofolio_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-studio.admin.home.data.portofolio.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-studio.admin.home.data.portofolio.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_gambar').attr('src', "{{ asset('images/landingpage_razenstudio/home/portofolio') }}" + '/' + data.result.gambar);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-studio.admin.home.data.portofolio.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    var lokasi_gambar = "{{ asset('images/landingpage_razenstudio/home/portofolio') }}"+'/'+data.result.gambar;
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-studio.admin.home.data.portofolio.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            $.ajax({
                url: url,
                beforeSend: function(){
