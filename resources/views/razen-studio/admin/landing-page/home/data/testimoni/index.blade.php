...
@section('title', 'Admin - Razen Studio | Admin | Landing Page | Home | Data | Testimoni')
        ...
        <div class="col-12 col-md-7">
            ...
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('razen-studio.admin.home.data.testimoni.index') }}">Testimoni</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#testimoni_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-studio.admin.home.data.testimoni.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#testimoni_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('razen-studio.admin.home.data.testimoni.store') }}",
                    method: "POST",
            ...
            {
                $.ajax({
                    url: "{{ route('razen-studio.admin.home.data.testimoni.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-studio.admin.home.data.testimoni.show', ['id' => ":id"]) }}";
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-studio.admin.home.data.testimoni.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-studio.admin.home.data.testimoni.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            $.ajax({
                url: url,
                beforeSend: function(){
