...
@section('title', 'Admin - Razen Studio | Admin | Portofolio Video Animasi')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">Portofolio Video Animasi</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="{{ route('razen-studio.admin.dashboard.index') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('razen-studio.admin.portofolio-video-animasi.index') }}">Portofolio Video Animasi</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="portofolio_video_animasi_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="portofolio_video_animasi_form" method="POST" enctype="multipart/form-data">
                        @csrf
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#portofolio_video_animasi_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-studio.admin.portofolio-video-animasi.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#create').click(function(){
            $('#portofolio_video_animasi_form')[0].reset();
            $('.dropify-clear').click();
        ...
        $('#portofolio_video_animasi_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-studio.admin.portofolio-video-animasi.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#portofolio_video_animasi_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#portofolio_video_animasi_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-studio.admin.portofolio-video-animasi.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-studio.admin.portofolio-video-animasi.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_link').text(data.result.link);
                    $('#detail_gambar').attr('src', "{{ asset('images/razen-studio/portofolio-video-animasi') }}" + '/' + data.result.gambar);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-studio.admin.portofolio-video-animasi.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    var lokasi_gambar = "{{ asset('images/razen-studio/portofolio-video-animasi') }}"+'/'+data.result.gambar;
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-studio.admin.portofolio-video-animasi.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            $.ajax({
                url: url,
                beforeSend: function(){
